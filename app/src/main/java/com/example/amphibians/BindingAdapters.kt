...
/**
 * mengUpdate data ang akan tapil pada RecyclerView
 */
// recyclerview
...
/**
 * binding adapter  untuk menampilkan [AmphibianApiStatus] pada perminataan jaringan untuk menampilkan image view.
 * akan menampilkan loading_animation. jika request error,
 * menampilkan gambar yang rusak jika terjadi  kesalahan koneksi. Ketika permintaan selesai
...
fun bindStatus(statusImageView: ImageView, status: AmphibianApiStatus?) {
    when (status) {
        ...
        AmphibianApiStatus.ERROR -> {
            statusImageView.visibility = View.VISIBLE
            // akn menampilkan gambar yang rusak jika terjadi kegagalna koneksi
